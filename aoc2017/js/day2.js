(function($,_){
  ...
  var solveFn1 = function(input) { /* FN1 START */
    ...
    for(let line of input.split(/\r|\n/)) {
      let row = [];
      for(let numberStr of line.split(/[ \t]+/)) {
    ...
    for(let row of matrix) {
      for(let i=0; i<row.length; i++) {
        for(let j=i+1; j<row.length; j++) {
          let numA = Math.max(row[i], row[j]);
          let numB = Math.min(row[i], row[j]);
          if(numA % numB === 0) {
            output +=  numA / numB;
          }
        }
  ...
  window.aoc[2] = [input, solveFn1, solveFn2];
}($,_))